...
const ScrollList = memo((props: Props) => {
  ...
  useEffect(() => {
    let timer: any;
    let scrollTop1 = 0;
   let Timetout = setTimeout(() => {
      let childHeight = Child1Ref.current.offsetHeight;
      let PersonHeight = PersonRef.current.clientHeight;
      if (isScrolle) {
        if (childHeight < PersonHeight) return;
                ...
                {waring.map((item, index) => {
                  return (
                    <li key={index} className={styles.gDhang0}>
                      ...
                      <div
                        className={styles.gDhang2}
                        style={{ flex: '4', fontSize: '13px' }}
                      >
                        {item.time}
                      </div>
