...
// }); //注册
hdU3D.init = function() {
	$(".sc-container").prepend('<iframe src="./hd_ZongLan/index.html?v=' + version +
		'" width="100%" frameborder="0" width="100%" height="100%"></iframe>');
	hdU3D.inform(); //位置信息
...
hdU3D.U3Dinit = function(gameInstance) {
	$('.U3DBtn').on('click', function() {
		if ($(this).hasClass('U3DSingleOn')) {
			$(this).removeClass('U3DSingleOn');
			gameInstance.SendMessage("RecvJsMessageObject", "GetEvent", "15001, ");
		} else {
			$(this).addClass('U3DSingleOn');
			gameInstance.SendMessage("RecvJsMessageObject", "GetEvent", "15001, ");
		}
	})
	$.post(hdInterface.selectSafetyMonitoring, {
		...
		if (data['code'] == 0) {
			gameInstance.SendMessage("RecvJsMessageObject", "GetEvent", "15002, " + JSON.stringify(data));
		}
	})
}
//一分钟更新
...
hdU3D.changeEnergy = function(){
	...
	$('.energyBtnParent .btn').on('click',function(){
		...
		num = index;
		$(`.content[data-type=${index}]`).removeClass('op').siblings('.content').addClass('op');
	})
	timerEnergy = setInterval(function(){
		num++;
		if(num%2==1){
			$('.energyBtnParent .btn[data-type=1]').addClass('on').siblings().removeClass('on');
			$(`.content[data-type=1]`).removeClass('op').siblings('.content').addClass('op');
		} else if(num%2==0){
			$('.energyBtnParent .btn[data-type=2]').addClass('on').siblings().removeClass('on');
			$(`.content[data-type=2]`).removeClass('op').siblings('.content').addClass('op');
		}
	},30*1000)
}
// 告警统计
hdU3D.projectSecurity = function() {
	// 报警
	$.get(hdInterface.alarmProjectNoticeList, {
		projectId: _PROJECT_ID,
		alarmTypes:'4,5,6'
	}, function(data) {
		if (data.code == 0) {
...
hdU3D.showHideButtom = function() {
	bottomBar = new SlideMenu({
		...
		callBackFun: function(e) {
			if (e) {
				$('.buttomIcon i').addClass('layui-icon-down').removeClass('layui-icon-up')
			} else {
...
hdU3D.windowTo = function() {
	$('#hoverFire').on('click', function() {
		...
		parent.$('dt[data-name=AreaManagement] a').each((i,e) => {
			...
			if(dataUrl.indexOf('bussiness/hdSquare/index.html') != -1){
				id = $(e).attr('data-id');
		...
		let url = `bussiness/hdSecurity/index.html?showmenu=show&siteId=${siteId}&projectId=${projectId}&titlename=%E5%8D%8E%E5%BE%B7%E5%B9%BF%E5%9C%BA%E5%8F%98%E7%94%B5%E6%89%80%E5%AE%89%E5%85%A8%E6%A6%82%E5%86%B5&menuLeftId=${menuLeftId}&v=${version}`;
	...
	$('#chuangan').on('click', function() {
		...
		let url = `/bussiness/equipmentList/index.html?showmenu=show&siteId=${siteId}&projectId=${projectId}&titlename=%E5%8D%8E%E5%BE%B7%E5%B9%BF%E5%9C%BA%E5%8F%98%E7%94%B5%E6%89%80%E8%AE%BE%E5%A4%87%E6%B8%85%E5%8D%95&menuLeftId=${menuLeftId}&v=${version}`;
		top.window.openNewTab(url, id, title);
	})
