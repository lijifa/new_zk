...
conDiagram.ImvEnergyConsumptionData = function () {
  $.get(
    ...
    function (data) {
      if (data['code'] === 0) {
        $('#electricityDay').html(
          convert(data.data.electricityDay) == '--'
            ? '0'
            : convert(data.data.electricityDay),
        );
        $('#load').html(convert(data.data.load));
        $('#maxLoad').html(convert(data.data.maxLoad));
          ...
          item.plctemplateInfos.forEach((sonItem) => {
            switch (sonItem.plcTypeId) {
              ...
              case '245': //受电回路c相电流
                ...
                break;
              case '304': //受电回路三相不平衡
                trinomialIrregularity = convertVal(
                  sonItem.refeStatus,
                  sonItem.refeValue,
                );
      ...
      data.data.forEach((item, index) => {
        if (item.deviceName != '') {
          $('#energyTransformer').append(`
						<div class="btn ${
              conDiagram.idMonitor == index ? 'on' : ''
            }" data-id="${index}">${item.deviceName}</div>
					`);
        }
      });
      conDiagram.powerDetection = data.data.length == 0 ? [temp] : data.data;
      Object.keys(conDiagram.powerDetection[conDiagram.idMonitor]).forEach(
        (item, index) => {
          if (
            item != 'deviceName' &&
            item != 'totalVoltage' &&
            ...
            echarsComponent.dashBoardEcharts({
              ...
              detailOffset: [0, 30],
              value:
                conDiagram.powerDetection[conDiagram.idMonitor][item] == null
                  ? 0
                  : Number(
                      convert(
                        conDiagram.powerDetection[conDiagram.idMonitor][item],
                      ),
                    ).toFixed(2),
              name: arr[item],
              pointer: {
...
function changeImgFan(flag, Redtext, Greentext) {
  //标志，异常，稳定
  switch (flag) {
    case 'o_alarm_type':
      return `<div class="gDtext">
