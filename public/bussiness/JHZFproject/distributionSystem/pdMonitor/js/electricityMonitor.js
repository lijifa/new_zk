...
electricityMonitor.getElectChars = function(obj) {
	...
	let params = {
		...
		interval: 1, //间隔[非必填]
		"yAxisMin":Math.floor(min),//y轴最小刻度默认为数据中最小值[非必填]
		"unit": unit[index], //单位
		grid: {
			left: '35px',
			right: '35px',
	...
	if(type == 'lowVoltagePowerFactor'){			// 功率因数
		params.yAxisMax = 1;
		params.yAxisMin = 0.7;
	} else if (type == 'lowVoltageItem'){			// 线电压
		params.yAxisMax = 260;
		params.yAxisMin = 200;
	} else if (type == 'lowVoltageLine'){			// 线电压
		params.yAxisMax = 450;
		params.yAxisMin = 300;
	} else if (type == 'lowVoltageFrequency'){		// 频率
		params.yAxisMax = 52;
		params.yAxisMin = 48;
	}
