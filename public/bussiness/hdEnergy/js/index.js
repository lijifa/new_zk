...
hdEnergy.electricityUsing = function() {
	$.get(hdInterface.selectImvElectricityStatistics,{
		"siteId":_SITE_ID,
	},function(data){
		if(data['code'] === 0){
				$('#hightSum').html(convert(data.data['hightSum'])=='--'?'0':convert(data.data['hightSum']));
				$('#highProportion').html(convert(data.data['highProportion']));
				$('#higtCostSum').html(convert(data.data['higtCostSum']));
				$('#flatSum').html(convert(data.data['flatSum'])=='--'?'0':convert(data.data['flatSum']));
				$('#flatProportion').html(convert(data.data['flatProportion']));
				$('#flatCostSum').html(convert(data.data['flatCostSum']));
				$('#lowEbbSum').html(convert(data.data['lowEbbSum'])=='--'?'0':convert(data.data['lowEbbSum']));
...
hdEnergy.transformerPower = function(){
	$.get(hdInterface.transformerUnPower,{
		"siteId":_SITE_ID,
		"type":0
	},function(data){
		if(data['code'] === 0){
	...
	$.get(hdInterface.selectMaximum,{siteId:_SITE_ID},function(data){
		if(data.code == 0){
			...
			hdEnergy.selectMaximum.forEach((item,index) => {
				...
				<button data-type="${index}" class="${index == hdEnergy.maxTrendBtnType ? 'on' : ''}">${item.name}</button>
				`)
			})
...
hdEnergy.maxTrendLine = function(type){
	...
	let name = [],dataX = [],dataY2 = [],dataY3 = [],dataY4 = [];
	data.data.dataX.split(',').forEach((item) => {
		dataX.push(item+':00');
	})
	dataY2 = (JSON.parse(data.data.dataY2).data).split(',');
...
hdEnergy.deviceRunList = function(){
	$.get(hdInterface.energyCentralizedReading,{
		"siteId":_SITE_ID,
	},function(data){
		if(data['code'] === 0){
			if(data.data.length == 0){
...
hdEnergy.betterSystom = function(){
	$.get(hdInterface.selectPowerConsumption,{
		"siteId":_SITE_ID
	},function(data){
		if(data['code'] === 0){
			if(data.data.length == 0){
							...
							<div class="betterBox">
								...
								<div class="betterCon">
									<div>
										...
										<em class="fontsize24 testColorLightBlue">${convert(item.powerConsumptionDay)}</em>
										kW·h</span>
										<span class="betterRight">占总电量：<em>${convert(item.proportion)}</em>%</span>
									</div>
									<div>
										<span class="betterLeft">当月: 
										<em class="testColorLightBlue">${convert(item.powerConsumptionMonth)}</em> kW·h</span>
...
function changeText(type){
	...
	switch (flag){
		case 0: img = 'image/down1.png';
			return ''+Math.abs(Number(type-0));//下降
		case 1: img = 'image/up1.png';
			...
			<li><div class="betterBox">
				...
				<div class="betterCon">
					<div class="betterLeft">
						<span>当日耗电量:</span>
						<span class="fontsize24 testColorLightBlue">${convert(item.powerConsumptionDay)}</span>
					...
					<div class="betterRight">
						...
						<img src="${img}" ></span>
						<span style="margin-top:10px;">本月: <em>${convert(item.powerConsumptionMonth)}</em>kW·h</span>
						<span>环比: 
