    ...
    if (data['code'] === 0) {
      ...
      if (data.data.length == 0) {
        ...
        $('#' + type + 'warnRuleTop5').append(
          '<tr class="nodata"><td colspan="4"><img src="../../common/image/nodata.png" class="nodateimg" style="margin-top:50px"/></td></tr>',
        );
      } else {
        data.data.forEach(function (item, ind) {
          $('#' + type + 'warnRuleTop5').append(
            `<tr class="${ind % 2 === 0 ? 'content1' : 'content2'}"><td>TOP${
              ind + 1
            }</td><td>${convert(
              item.reason,
            )}</td><td class="tdContent">${convert(item.total)}</td></tr>`,
          );
        });
      }
    }
  ...
  $.post(hdInterface.selectAlarmTop, {
    ...
    if (data['code'] === 0) {
      ...
      if (data.data.length == 0) {
        ...
        data.data.forEach(function (item, ind) {
          $('#' + type + 'warnRuleLast5').append(
            `<tr class="${ind % 2 === 0 ? 'content1' : 'content2'}"><td>LAST${
              ind + 1
            }</td><td>${convert(
              item.reason,
            )}</td><td class="tdContent">${convert(item.total)}</td></tr>`,
...
securityStatistical.maxAmbientHumidity = function (type) {
  $.get(
    hdInterface.maxAmbientHumidity,
    {
      siteId: _SITE_ID,
    },
    function (data) {
      if (data['code'] === 0) {
        if (JSON.stringify(data.data) === '{}') {
          $('#' + type + 'AlarmRank1>.hd-loadmask').html(
            `<img src="../../common/image/nodata.png" class="position50" style="width:150px;height: 100px;">`,
          );
        } else {
          let title = [];
...
securityStatistical.maxDeviceTemperature = function (type) {
  $.get(
    hdInterface.maxDeviceTemperature,
    {
      siteId: _SITE_ID,
    },
    function (data) {
      if (data['code'] === 0) {
        if (JSON.stringify(data.data) === '{}') {
          $('#' + type + 'AlarmRank2>.hd-loadmask').html(
            `<img src="../../common/image/nodata.png" class="position50" style="width:150px;height: 100px;">`,
          );
        } else {
          let title = [];
        ...
        if (data.data.length == 0) {
          ...
          data.data.forEach((item) => {
            let obj = {
              ...
              state:
                convert(item.state) == '1'
                  ? '报警'
                  : convert(item.state) == '0'
                  ? '已修复'
                  : '--',
            };
