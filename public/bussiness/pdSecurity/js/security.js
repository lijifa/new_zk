      ...
      for (let key in data.data) {
        ...
        if (data.data[key].length !== 0) {
          ...
          if (
            ...
            key == 'transformerTemperature' ||
            key == 'cableTemperature' ||
            key == 'electricLeakage'
          ) {
    ...
    function (data) {
      ...
      if (data['code'] === 0) {
        if (data.data.length == 0) {
          ...
          let transformerMonitor = new listRollNew(
            'transformerMonitor',
            paramTransformerMonitor,
          );
        }
      }
    },
...
security.alarmRate = function (data) {
  var value =
    (data.data.realWarnRuleCount / data.data.warnRuleCount) * 100 || '';
  var showText = `<span style="font-size:30px;">${convert(
    data.data.currentMonthCount,
  )}</span>`;
  // console.log("实时预警规则====");
...
security.integrityRate = function (data) {
  ...
  var showText = `<span style="color:#ff4444;"><span style="font-size:30px;">${convert(
    data.data.currentDayCount,
  )}</span></span>`;
  // console.log("=======实时设备故障");
  // console.log(showText);
