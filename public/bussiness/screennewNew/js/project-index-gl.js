  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.code == 0) {
        ...
        if (data.length == 0) {
          $('#main').html(
            '<div class="nodata"><img src="../../common/image/nodata.png" class="nodateimg"></div>',
          );
          $('#humidity').hide();
          return;
        } else {
...
projectindex.loadSystemDifference = function () {
  $.ajax({
    ...
    url: hdInterface.systemDifferenceDetectionModule,
    type: 'POST',
    data: {
      siteId: siteId,
    },
    success: function (result) {
      var data = result.data || [];
      if (result['code'] === 0) {
        $('#differenceManifold').html(
          convert(data.pressureDifferenceOfWaterCollectorAndDistributor),
        ); //集分水器压差
        $('#SystemDifference').html(
          convert(data.pressureDifferenceOfRefrigeratingSupplyAndReturn),
        ); //系统压差
        $('#supplyAndReturn').html(
          convert(data.temperatureDifferenceBetweenChilledSupplyAndReturnWater),
        ); //供回水温差
          ...
          deviceStatusList.forEach((item, index) => {
            ...
            item.plctemplateInfos.forEach((sonItem) => {
              ...
              if (sonItem.plcTypeId == '313') {
                console.log('---', item);
                if (
                  sonItem.refeStatus == 'stop_type' ||
                  sonItem.refeStatus == 'start_type'
                ) {
                  state = checkStatus(sonItem.refeStatus, sonItem.refeText);
