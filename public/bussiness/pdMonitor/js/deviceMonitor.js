...
deviceMonitor.transformer = function () {
  ...
  $('.transformer').on('click', 'button', function () {
    ...
    $('.power_temperature button')
      .eq(0)
      .addClass('on')
      .siblings()
      .removeClass('on');
    deviceMonitor.transformerFlag = $(this).index();
    deviceMonitor.changeTable(
      deviceMonitor.imvTransformerList[deviceMonitor.transformerFlag],
    ); //渲染table表格
    changeIDTYPE();
  ...
  function changeIDTYPE() {
    ...
    if (id == deviceMonitor.idFlag && type == deviceMonitor.typeFlag) {
      ...
      deviceMonitor.selectTransformerChart(
        deviceMonitor.idFlag,
        deviceMonitor.typeFlag,
      );
    }
  }
};
    ...
    function (data) {
      if (data['code'] === 0) {
        if (data.data.length == 0) {
          ...
          let switchMonitor = new listRollNew(
            'switchMonitor',
            paramSwitchMonitor,
          );
        }
