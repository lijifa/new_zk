...
document.addEventListener("visibilitychange", function () {
	if (document.visibilityState == 'hidden') {
		...
		monitoring.oneMinutesUpdate(); //一分钟更新
		monitoring.fiveMinutesUpdate(); //五分钟更新
		monitoring.upData();
	}
});
var objName = layuiId;
...
monitoring.u3dinit = function (gameInstance) {
	...
	let newCamera = new camera({
		outerContainer: '#camera-containerOT',
		cameraIdName: 'camera0',
	...
	$.post(hdInterface.selectInsideTemperatureCurve, {
		...
		if (res.code == 0) {
			...
			echarsComponent.getLine({
				...
				// ydata:[datay,datay2,datay3],//纵坐标数据[必填]
				ydata: [datay3, datay, datay2], //纵坐标数据[必填]
				ydataTitle: ['室外', '供水', '回水'],
				// linecolors:['#0A94FF','#FFB72C','#48D396'],//线颜色[非必填]
...
monitoring.windowTo = function () {
	$('#windowTo').on('click', function () {
		...
		parent.$('#hidenavbar a').each((i, e) => {
			if ($(e).attr('data-url').indexOf('/equipmentList/index.html') != -1) {
				url = $(e).attr('data-url');
			}
		})
		let id = parent.$('.hd-tab.layui-tab-title .layui-this').attr('lay-id');
		let title = '光合谷地热井机房';
		monitoring.openNewTab(url, id, title);
	})
}
monitoring.init();
