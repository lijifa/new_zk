...
energySavingStatistical.statistics30 = function () {
  //能耗总计
  $.post(
    hdInterface.AllenergyStaticsModule,
    {
      siteId: _SITE_ID,
      timeCode: timeCode,
    },
  ...
  //能耗总计表格
  $.post(
    hdInterface.AllenergyListModule,
    {
      siteId: _SITE_ID,
      timeCode: timeCode,
    },
    function (data) {
      if (data["code"] === 0) {
        ...
        if (data.data.length == 0) {
          $("#ul-statistics .hd-loadmask").html(
            `<img src="../../common/image/nodata.png" class="iconimg" style="top: 60%;width: 150px;height: 100px;transform: translate(-40%);">`
          );
        } else {
          var param = {
            vis: "10", //可视数
    ...
    function (data) {
      if (data["code"] === 0) {
        ...
        if (data.data.length === 0) {
          ...
          $("#main_top .hd-loadmask").html(
            `<img src="../../common/image/nodata.png" class="imagePo">`
          );
        } else {
          var list = [];
          data.data.forEach((item, index) => {
  ...
  $.post(
    ...
    function (data) {
      if (data["code"] === 0) {
        // console.log(data.data)
        if (data.data.length == 0) {
          $("#ul-statistics1 .hd-loadmask").html(
            `<img src="../../common/image/nodata.png" class="iconimg" style="top: 60%;width: 150px;height: 100px;transform: translate(-40%);">`
          );
        } else {
          var param1 = {
...
energySavingStatistical.topList = function (selector, data, electric) {
  ...
  data.forEach((item, index) => {
    maxElectric =
      item.electric - 0 > maxElectric ? item.electric - 0 : maxElectric;
  });
